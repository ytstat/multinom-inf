...
filename <- paste("/moto/home/yt2661/work/multinom-inf/experiments/multinomial_model_3/result/", seed, ".RData", sep = "")
...
for (i in 1:length(n)) {
  ...
  beta_coef[ind1+1, 1] <- c(1, 1, 1)*sig.strength
  beta_coef[ind2+1, 2] <- c(1, 1, 1)*sig.strength
  ...
  fit <- cv.pemultinom(x, y, nfolds = 5, nlambda = 100, ncores = ncores)
  beta <- fit$beta.min
